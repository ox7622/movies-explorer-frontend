    ...
    return (<nav className={`nav ${isOpen ? "nav_open" : ""} ${burgerMenu ? "nav_mobile" : "nav_desktop"} `}>
        ...
        <div className="nav__container">
            <ul className="nav__links">
                <li className="nav__list-item"><Link to={'/'} onClick={handleClick} className='nav__link  hover-link hover-opacity'> Главная </Link></li>
                {children}
            </ul>
            <Link to={'/profile'} onClick={handleClick} className="nav__link  nav__link_account hover-button">Аккаунт</Link>
        </div>
    </nav>
    )
}
